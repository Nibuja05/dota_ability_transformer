...
const transformEnums_1 = require("./transformEnums");
...
function getAbilityPath() {
    ...
    if (!fs.existsSync(abilityDir))
        throw Error("NPC script path not found");
 ...
 * @returns generated abilities object
 */
function getGeneratedAbilities(path) {
 ...
 */
function writeGeneratedAbilities(path, abilityObject) {
...
function getAbilityPathName(path) {
    const abilityPath = getAbilityPath();
    let abilityFilePath;
    switch (configuration.modularization) {
        case "none" /* None */:
 ...
 */
function removeAbility(path, abilityName, remBase) {
...
function getCleanedFilePath(node) {
    ...
    if (!match)
        throw Error("Invalid File Path: " + absPath);
    return match[1] + ".lua";
}
/**
...
function getAbilityFileCountByFolder(path) {
